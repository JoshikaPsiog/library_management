...
void exportReportsToCSV() {
    ...
    ofstream topBooksFile(basePath + "top_issued_books.csv");
    if (!topBooksFile.is_open()) {
        cout << "Failed to create top_issued_books.csv" << endl;
    } else {
        topBooksFile << "BookID,Title,IssueCount\n";
        for (const auto& row : topBooks) {
            topBooksFile << row[0] << "," << escapeCSV(row[1]) << "," << row[2] << "\n";
        }
        topBooksFile.close();
        cout << "Exported Top Issued Books to " << basePath << "top_issued_books.csv" << endl;
    }
    ...
    ofstream activeMembersFile(basePath + "active_members.csv");
    if (!activeMembersFile.is_open()) {
        cout << "Failed to create active_members.csv" << endl;
    } else {
        activeMembersFile << "MemberID,Name,BooksIssued\n";
        for (const auto& row : activeMembers) {
            activeMembersFile << row[0] << "," << escapeCSV(row[1]) << "," << row[2] << "\n";
        }
        activeMembersFile.close();
        cout << "Exported Active Members to " << basePath << "active_members.csv" << endl;
    }
    ...
    ofstream fineSummaryFile(basePath + "fine_summary.csv");
    if (!fineSummaryFile.is_open()) {
        cout << "Failed to create fine_summary.csv" << endl;
    } else {
        fineSummaryFile << "MemberID,Name,TotalFine\n";
        for (const auto& row : fineSummary) {
            fineSummaryFile << row[0] << "," << escapeCSV(row[1]) << "," << row[2] << "\n";
        }
        fineSummaryFile.close();
        cout << "Exported Fine Summary to " << basePath << "fine_summary.csv" << endl;
    }
}
bool login() {
    int roleChoice;
...
void showPaginated(const vector<vector<string>>& data, const string& type) {
    ...
    do {
        ...
        int end = min(start + pageSize, (int)data.size());
        if (type == "Books") {
            cout << left << setw(5) << "ID"
                 << setw(25) << "Title"
                 << setw(20) << "Authors"
            ...
            cout << left << setw(8) << "ID" << setw(30) << "Name" << setw(30) << "Email" << setw(10) << "Type" << endl;
            cout << string(78, '-') << endl;
            for (int i = start; i < end; ++i) {
                string name = data[i][1].substr(0, 29);
                string email = data[i][2].substr(0, 29);
                cout << left << setw(8) << data[i][0] << setw(30) << name << setw(30) << email << setw(10) << data[i][3] << endl;
            }
        }
        else if (type == "Transactions") {
            cout << left << setw(8) << "ID" << setw(10) << "BookID" << setw(10) << "MemberID" << setw(12) << "IssueDate" << setw(12) << "DueDate" << setw(10) << "Status" << setw(8) << "Fine" << endl;
            cout << string(78, '-') << endl;
            for (int i = start; i < end; ++i) {
            ...
            cout << left << setw(8) << "BookID" << setw(30) << "Title" << setw(12) << "IssueCount" << endl;
            cout << string(50, '-') << endl;
            for (int i = start; i < end; ++i) {
                string title = data[i][1].substr(0, 29);
                cout << left << setw(8) << data[i][0] << setw(30) << title << setw(12) << data[i][2] << endl;
            }
        }
        else if (type == "ActiveMembers" || type == "Fines") {
            string label = (type == "ActiveMembers") ? "BooksIssued" : "TotalFine";
            cout << left << setw(10) << "MemberID" << setw(30) << "Name" << setw(15) << label << endl;
            cout << string(55, '-') << endl;
            for (int i = start; i < end; ++i) {
                string name = data[i][1].substr(0, 29);
                cout << left << setw(10) << data[i][0] << setw(30) << name << setw(15) << data[i][2] << endl;
            }
        }
int totalPages = (data.size() + pageSize - 1) / pageSize;
        ...
        if (choice == 'N' && end < (int)data.size()) ++page;
...
void addBook() {
    ...
    if ((availability != "Yes"||availability != "yes") && (availability != "No"||availability != "no")) {
...
void updateBook() {
    ...
    cout << "Enter new Title (blank to skip): "; cin.ignore(); getline(cin, title);
    cout << "Enter new Authors (blank to skip): "; getline(cin, authors);
    cout << "Enter new ISBN (blank to skip): "; cin >> isbn;
    string query = "UPDATE dbo.Books SET ";
    bool hasUpdate = false;
    if (!title.empty()) {
        for (size_t i = 0; i < title.length(); ++i) {
            if (title[i] == '\'') title.insert(i++, "'");
        }
        query += "Title = '" + title + "'";
        hasUpdate = true;
    }
    ...
    if (runQuery(query)) cout << "Book updated!" << endl;
    else cout << "Failed to update book." << endl;
}
...
void deleteBook() {
    ...
    if (runQuery(query)) cout << "Book deleted!" << endl;
    else cout << "Failed to delete book." << endl;
}
...
void searchBooks() {
    string value;
    cout << "Enter Title or Author to search: "; cin.ignore(); getline(cin, value);
    ...
    while (attempts > 0) {
        ...
        if (!file.is_open()) {
            ...
            if (attempts == 0) {
                ...
                cout << "Attempts remaining: " << attempts << ". Please ensure books.csv exists in " << cwd << " or enter the full path (or 'q' to quit): ";
                cin >> filename;
                if (filename == "q") {
                    cout << "Bulk import aborted." << endl;
                    return;
...
void addMember() {
    ...
    cout << "Enter Type (Regular/Premium): "; cin >> type;
    ...
    if (name.empty() || email.empty() || pass.empty()) {
        cout << "Name, Email, and Password are required!" << endl;
        return;
    }
    for (size_t i = 0; i < name.length(); ++i) {
        if (name[i] == '\'') name.insert(i++, "'");
    }
    for (size_t i = 0; i < email.length(); ++i) {
        if (email[i] == '\'') email.insert(i++, "'");
    }
    ...
    string query = "INSERT INTO dbo.Members (Name, Email, MembershipType, Role, Password) "
                   "VALUES ('" + name + "', '" + email + "', '" + type + "', '" + role + "', '" + pass + "')";
    if (runQuery(query)) cout << "Member added!" << endl;
    else cout << "Failed to add member." << endl;
}
...
void updateMember() {
    ...
    cout << "Enter new Name (blank to skip): "; cin.ignore(); getline(cin, name);
    cout << "Enter new Email (blank to skip): "; getline(cin, email);
    cout << "Enter new Type (Regular/Premium, blank to skip): "; cin >> type;
    ...
    if (!email.empty()) {
        ...
        auto res = getResults("SELECT Email FROM dbo.Members WHERE Email = '" + email + "' AND MemberID != '" + memberID + "'");
        if (!res.empty()) {
            cout << "Email already exists!" << endl;
            return;
        }
        query += (hasUpdate ? ", " : "") + string("Email = '") + email + "'";
        hasUpdate = true;
    }
    if (!type.empty()) {
        if (type != "Regular" && type != "Premium") {
        ...
        query += (hasUpdate ? ", " : "") + string("MembershipType = '") + type + "'";
        hasUpdate = true;
    }
    if (!hasUpdate) {
        cout << "No changes provided!" << endl;
    ...
    if (runQuery(query)) cout << "Member updated!" << endl;
    else cout << "Failed to update member." << endl;
}
...
void deleteMember() {
    ...
    if (runQuery(query)) cout << "Member deleted!" << endl;
    else cout << "Failed to delete member." << endl;
}
...
void searchMembers() {
    string value;
    cout << "Enter Name or Email to search: "; cin.ignore(); getline(cin, value);
    for (size_t i = 0; i < value.length(); ++i) {
        if (value[i] == '\'') value.insert(i++, "'");
    }
    string query = "SELECT MemberID, Name, Email, MembershipType FROM dbo.Members WHERE Name LIKE '%" + value + "%' OR Email LIKE '%" + value + "%'";
    auto res = getResults(query);
    showPaginated(res, "Members");
}
...
void issueBook() {
    ...
    auto issuedBooks = getResults("SELECT COUNT(*) FROM dbo.Transactions WHERE MemberID = '" + memberID + "' AND Status = 'Issued'");
    if (!issuedBooks.empty() && stoi(issuedBooks[0][0]) >= config.maxBooksPerMember) {
        cout << "Member has reached max books limit (" << config.maxBooksPerMember << ")!" << endl;
        return;
    }
    string query = "INSERT INTO dbo.Transactions (BookID, MemberID, IssueDate, DueDate, Status) VALUES ('" + bookID + "', '" + memberID + "', GETDATE(), DATEADD(day, " + to_string(config.reservationDurationDays) + ", GETDATE()), 'Issued')";
    string updateBook = "UPDATE dbo.Books SET Availability = 'No' WHERE BookID = '" + bookID + "'";
    bool success = true;
    SQLSetConnectAttr(connHandle, SQL_ATTR_AUTOCOMMIT, (SQLPOINTER)SQL_AUTOCOMMIT_OFF, 0);
    if (!runQuery(query)) success = false;
...
void returnBook() {
    ...
    cout << "Enter TransactionID: "; cin >> transactionID;
    if (!all_of(transactionID.begin(), transactionID.end(), ::isdigit)) {
        cout << "TransactionID must be numeric!" << endl;
        return;
    }
    auto res = getResults("SELECT BookID, Status FROM dbo.Transactions WHERE TransactionID = '" + transactionID + "' AND Status = 'Issued'");
    if (res.empty()) {
        cout << "Transaction not found or not issued!" << endl;
        return;
    }
    ...
    string query =
        ...
        "Status = 'Returned', "
        "ReturnDate = GETDATE(), "
        "FineAmount = CASE WHEN GETDATE() > DueDate THEN DATEDIFF(day, DueDate, GETDATE()) * "
        + to_string(config.fineRate) + " ELSE 0 END "
...
void reserveBook() {
    ...
    if (bookRes[0][0] == "Yes") {
        cout << "Book is available, issue it instead!" << endl;
        return;
    }
    Config config = getConfig();
    string query = "INSERT INTO dbo.Transactions (BookID, MemberID, IssueDate, DueDate, Status) VALUES ('" + bookID + "', '" + memberID + "', GETDATE(), DATEADD(day, " + to_string(config.reservationDurationDays) + ", GETDATE()), 'Reserved')";
    if (runQuery(query)) cout << "Book reserved!" << endl;
    else cout << "Failed to reserve book." << endl;
}
...
void viewHistory() {
    ...
    string query = "SELECT TransactionID, BookID, MemberID, IssueDate, DueDate, Status, FineAmount "
                   "FROM dbo.Transactions "
                   "WHERE MemberID = " + memberID + " AND Status != 'Returned' "
    ...
    if (res.empty()) {
        cout << "No active transactions found for MemberID " << memberID << endl;
        return;
    }
    showPaginated(res, "Transactions");
}
